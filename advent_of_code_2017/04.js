function count_valid_passphrases(input, validation_fn) {
  return input.split('\n').filter((pp) => validation_fn(pp)).length;
}

function validate_no_duplicates(passphrase) {
  const words = passphrase.split(' ');
  return words.length === words.filter((word, idx, arr) => idx === arr.indexOf(word)).length;
}

function validate_no_anagrams(passphrase) {
  const words = passphrase.split(' ');
...
run_tests(validate_no_duplicates, test_suite_no_duplicates);
run_tests(validate_no_anagrams, test_suite_no_anagrams);
...
console.log(`Part 1: ${count_valid_passphrases(input, validate_no_duplicates)}`);
console.log(`Part 2: ${count_valid_passphrases(input, validate_no_anagrams)}`);
